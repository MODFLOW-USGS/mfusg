      MODULE GWFEVTMODULE
        INTEGER, SAVE, POINTER    ::NEVTOP,IEVTCB,MXNDEVT,INIEVT,mxznevt
...
C     ------------------------------------------------------------------
     ...
     1      IEVT,MXNDEVT,INIEVT,EVTF,ETFACTOR,iznevt,mxznevt
...
C1------ALLOCATE SCALAR VARIABLES.
      ALLOCATE(NEVTOP,IEVTCB,MXNDEVT,INIEVT,mxznevt)
...
C3B------INSURF=>0, SO READ SURFACE.
        IF(IUNSTR.EQ.0)THEN
          ...
          ENDDO
        ELSE
          CALL U2DREL(SURF,ANAME(2),1,INIEVT,0,IN,IOUT)
        ENDIF
      END IF
C
...
C
C4B1----THERE ARE NO PARAMETERS,SO READ EVTR USING U2DREL
          CALL U2DREL(EVTR,ANAME(3),1,INIEVT,0,IN,IOUT)
        ELSE
C
C4B2----DEFINE EVTR USING PARAMETERS. INEVTR IS THE NUMBER OF
C4B2----PARAMETERS TO USE THIS STRESS PERIOD.
          ...
          IF (INEVTR.EQ.0) THEN
            ...
            CALL USTOP(' ')
          END IF
          CALL UPARARRSUB2(EVTR,INIEVT,1,0,INEVTR,IN,IOUT,'EVT',
     1        ANAME(3),'EVT',IEVTPF)
        END IF
       ENDIF
C
...
C6B------IF INEXDP=>0 CALL MODULE U2DREL TO READ EXTINCTION DEPTH
          ...
          ENDDO
        ELSE
          CALL U2DREL(EXDP,ANAME(4),1,INIEVT,0,IN,IOUT)
        ENDIF
      END IF
C
...
C7B------IF INIEVT=>0 THEN READ INDICATOR ARRAY.
   ...
   57       CONTINUE
          ...
          ELSE ! FOR UNSTRUCTURED GRID
            CALL U2DINT(IEVT,ANAME(1),1,INIEVT,0,IN,IOUT)
          ENDIF
        END IF
...
!        ELSE
!          CALL U2DINT(iznevt,ANAME(5),1,INIEVT,0,IN,IOUT)  
!        ENDIF
!      ENDIF
C----------------------------------------------------------------
C
...
C     ------------------------------------------------------------------
     ...
     *                      IBOUND,NODLAY,IVC
      USE GWFEVTMODULE,ONLY:NEVTOP,EVTR,EXDP,SURF,IEVT,INIEVT
C
...
C13-----MODULE SAVE THEM.
     ...
     1        INIEVT,NEVTOP,NODES,IOUT,DELT,PERTIM,TOTIM,IBOUND)
      ENDIF
C
