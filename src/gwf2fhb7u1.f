...
C2A----IF CELL-BY-CELL FLOWS WILL BE SAVED AS A LIST, WRITE HEADER.
     ...
     1          NODES,NFHBGW,IOUT,DELT,PERTIM,TOTIM,IBOUND)
        ENDIF
        IF(ICLNCB.GT.0)
     1    CALL UBDSV4U(KSTP,KPER,TEXT,NAUX,'IFACE           ',ICLNCB,
     2                 NCLNNDS,NFHBCLN,IOUT,DELT,PERTIM,TOTIM,IBOUND)
        
!        IF(IUNSTR.EQ.0) THEN 
!          CALL UBDSV2(KSTP,KPER,TEXT,IFHBCB,NCOL,NROW,NLAY,
!     1          NFLW,IOUT,DELT,PERTIM,TOTIM,IBOUND)
!        ELSE
!          CALL UBDSV2U(KSTP,KPER,TEXT,IFHBCB,NEQS,
!     1          NFLW,IOUT,DELT,PERTIM,TOTIM,IBOUND)
!        ENDIF
      ENDIF
C
...
C6------RETURN THE ACTUAL FLOW IN THE BDFV ARRAY.
   97 CONTINUE
      IF(IBD.EQ.2)THEN
        XFACE(1)=IFLLOC(4,L)
        IF(ND.GT.NODES.AND.ICLNCB.GT.0) THEN
          CALL UBDSVBU(ICLNCB,NCLNNDS,ND-NODES,Q,XFACE,1,NAUX,
     1                 1,IBOUND)
        ELSE
          IF(IUNSTR.EQ.0)THEN
            IL = (ND-1) / (NCOL*NROW) + 1
  ...
  !          CALL UBDSVAU(IFHBCB,NEQS,ND,Q,IBOUND)
          ENDIF 
        ENDIF
      ENDIF
      BDFV(2,L)=Q
...
C7------CALL UBUDSV TO SAVE THEM
     ...
     1                          NLAY,IOUT)
        ELSE
          CALL UBUDSVU(KSTP,KPER,TEXT,IFHBCB,BUFF,NEQS,
     1                          IOUT,PERTIM,TOTIM)
        ENDIF
      ENDIF
C
